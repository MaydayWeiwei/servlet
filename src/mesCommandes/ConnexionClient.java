...
public class ConnexionClient extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		if (connexion == null) {
			...
			out.print("<form action=\"seconnecter\" method=GET>");
			if (inscrit == null) {
		...
		// ********************************************************************************************
			...
			if(motPasseRecu.equals(motPasseCookie)){
				response.sendRedirect("entrer?nom="+nomRecu);
			}
			else{
				response.sendRedirect("seconnecter?erreur=nom ou mot de passe incorrect");
			}
		}
